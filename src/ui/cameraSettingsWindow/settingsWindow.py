...
class SettingsWindow(QtWidgets.QWidget):
    ...
    def button_ok_func(self):
        try:
            ...
            # Saving the Settings
            if int(self.getlevel1l.text()) < 0 or \
                    int(self.getlevel1l.text()) > 100 or \
                    int(self.getlevel2l.text()) < 0 or \
                    int(self.getlevel2l.text()) > 100 or \
                    int(self.getlevel1l.text()) >= int(self.getlevel2l.text()):
                self.console.raise_text("Wrong values for image contrast.", 3)
            elif int(self.getcropxi_l.text()) > int(self.getcropxf_l.text()) or \
                ...
                self.cam.set_camera_settings(self.setField_temperature.text(), self.prel.text(), self.expl.text(),
                                             self.combo.currentIndex(), self.tempo_fotos.text(),
